...
const AllProduct = () => {
  ...
  const handleChangePageShowProduct = (e) => {
    let lengthBtnNagivationPage = document.getElementsByClassName(
      "pagination__products"
    )[0].children.length;

    for (let index = 0; index < lengthBtnNagivationPage; index++) {
      document.getElementsByClassName("pagination__products")[0].children[
        index
  ...
  const showAllProduct = (filterPara) => {
    ...
    for (let index = 0; index < limitProductNeedShow; index++) {
      let indexTemp = productFilterArray[index][0];
      let allImagePerProduct = Object.keys(
        json["product_" + indexTemp]["product_" + indexTemp + "_link_image"]
      ).length;
      console.log(allImagePerProduct);
      let linkImageProduct = `../../../assets/images/ImageAllProducts/product_${indexTemp}/product_${indexTemp}_1.png`;
      ...
      elementAllProduct.push(
        <div class="card__item">
          ...
          <p>
            <Link
              to={`/detailProduct?idproduct=${indexTemp}`}
              state={{
                productName:
                  json["product_" + indexTemp][
                    "product_" + indexTemp + "_name"
                  ],
                priceProduct:
                  json["product_" + indexTemp][
                    "product_" + indexTemp + "_price"
                  ],
                imageProduct: linkImageProduct,
                idImage: indexTemp,
                allImagesPerProduct: allImagePerProduct,
              }}
            >
              {json["product_" + indexTemp]["product_" + indexTemp + "_name"]}
            </Link>
          </p>
          <h3>
            {json["product_" + indexTemp]["product_" + indexTemp + "_price"]}
          </h3>
        </div>
      );
    }
